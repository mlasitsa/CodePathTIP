...
def smaller_than_current(nums):
    ...
    # Use a hashmap to store first occurrence index in the sorted array
    hmap = {}  
    for i, num in enumerate(sorted_nums):
        if num not in hmap:  # Only store the first occurrence
            hmap[num] = i  

    # Retrieve results in the original order
    return [hmap[num] for num in nums]  # O(N)
